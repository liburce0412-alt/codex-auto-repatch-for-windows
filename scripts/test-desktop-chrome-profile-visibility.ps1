...
foreach ($officialScript in @('check-extension-installed.js', 'chromium-browser-diagnostics.mjs', 'extension-ids.json')) {
  ...
  # Stream bytes instead of inheriting WindowsApps EFS metadata into the fixture.
  $destinationPath = Join-Path $cacheScriptsRoot $officialScript
  ...
  if ((Get-FileHash -LiteralPath $sourcePath).Hash -ne (Get-FileHash -LiteralPath $destinationPath).Hash) {
    throw "Official diagnostic fixture copy differs: $officialScript"
  }
}
