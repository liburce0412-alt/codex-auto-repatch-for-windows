...
try {
  ...
  $localMain917Source = $mainSlotSource.Replace(',computerUseNodeRepl:!0', '').Replace('browserExtensions:e.browserExtensions,', 'browserExtensions:e.browserExtensions,browserSettingsCloudSync:e.browserSettingsCloudSync,')
  $localMain917 = Invoke-PatcherFixture -Name 'codex-26-917-desktop-feature-main-local' -Source $localMain917Source -Target 'main' -ExpectedExitCode 0
  if ($localMain917.Output -cne 'patched') { throw '26.917 main fixture was not patched' }
  Assert-ValidJavaScript -Name '26.917 main' -AssetPath $localMain917.AssetPath
  Assert-Idempotent -Name '26.917 main' -AssetPath $localMain917.AssetPath -Target 'main'
