...
The local CUA patch combines PR #64 readiness recognition with migration of PR #62 output: modern bundles require the unique complete readiness predicate, legacy bundles retain `computerUseNodeRepl`, and existing patch markers alone never establish compatibility. Migration is idempotent. The local external update watcher shows a read-only progress terminal, requires a verified original Store MSIX before uninstalling Store, and cleans only this run's manifest-verified build files after stable restart. Missing recovery media preserves Store and leaves repatching incomplete. See [local post-update safety](references/local-post-update-safety.md); do not bypass that guard to finish a patch.
