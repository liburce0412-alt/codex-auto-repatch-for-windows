...
The default Browser/Computer Use scope and full repack also include `patch-browser-import-diagnostics.cjs`. It preserves import behavior while logging thrown import errors through the existing sensitive-error channel as `Browser profile import exception`; it does not add request/result logging. The user deferred installation of this diagnostic change until their next repack on 2026-09-25. Do not start a repack or restart solely to apply it. This is diagnostic instrumentation, not a verified fix for Chrome cookie import.

The local CUA patch combines PR #64 readiness recognition with migration of PR #62 output: modern bundles require the unique complete readiness predicate, legacy bundles retain `computerUseNodeRepl`, and existing patch markers alone never establish compatibility. Migration is idempotent. The local external update watcher shows a read-only progress terminal and cleans manifest-verified build files plus historical failed handoffs and unused older CLI copies after stable restart, retaining the current runtime and latest successful recovery package/data backup. On 2026-09-24 the user explicitly cancelled mandatory Store recovery media and requested automatic Codex CLI launch after patch failure. Official recovery media is optional; the signed patched artifact and independent CLI must still validate before replacement. See [local post-update safety](references/local-post-update-safety.md).
...
Classify the request before running helpers: diagnosis/status permits read-only checks, while repair permits the relevant scoped changes. `-VerifyOnly` can repair local state; for a read-only Computer Use check use `-StrictVerifyOnly`. Do not infer safety from a flag name. A full wrapper also installs, changes configuration and cleans up (see its defaults below); use it only when that concrete repair scope is authorized, preserving the requested package/version and existing model routing.
...
## Reviewed Desktop 26.924 integration

See [the integration record](references/upstream-20260926-integration.md) for reviewed upstream main and PRs, the actual staged-package dry run, and the remaining install/UI acceptance boundary. Automatic artifact preparation now passes `-PreserveSourceVersion` together with `-OnlyBrowserComputerUse -ForceRebuild`; the switch is forbidden with direct `-Install`. Signed outputs must pass block-map payload verification before their path is published to the external watcher. The existing hash, identity, signature, backup, fallback and stable-start cleanup checks still apply.

## External Executor For Desktop-Restarting Repairs

The generic manual MSIX installers use in-place updates. All three direct patcher entrypoints increment the package revision and use `scripts/lib/msix-safe-install.ps1`. The existing automatic exact-artifact watcher has a separate, previously authorized replacement contract documented in [local post-update safety](references/local-post-update-safety.md); do not silently substitute a revisioned package into that exact-version handoff. It checks the manifest, publisher, architecture, signature, version, administrator requirement for packaged services, and installer ancestry before deployment. Permission or same-version failures must stop without closing or removing the working app. Do not restore a `Remove-AppxPackage` fallback to work around a failed update.

`Start-Process` from Desktop can still belong to Desktop's job/process tree and terminate with it. Verify an external executor's ancestry and user session before releasing installation. Packages declaring `windows.service`, `packagedServices`, or `localSystemServices` require an elevated executor; a valid signature does not establish deployment permission. Obtain normal Windows UAC consent and verify the elevated token before any app shutdown. For a recoverable manual in-place update, keep a separately signed recovery package containing the original program files, with a revision higher than the intended update, until real launch and runtime acceptance pass. The automatic workflow retains its user-selected optional original Store media plus mandatory application-data backup and CLI fallback; this manual recovery guidance does not revoke that choice. The normal patcher entrypoints do not create that recovery package automatically; `Invoke-RecoverableMsixInstall` is available for an external executor that has prepared it and supplies real launch validation. See the deployment failure case in `references/restriction-debug-cases.md`.
...
- If a Computer Use task fails before app interaction with `Package subpath ... is not defined by "exports"`, `Module not found: @oai/sky`, missing `setupComputerUseRuntime`, or an internal `@oai/sky` / `computer_use_client_base` import path error, treat it as local bundled plugin/runtime drift. For diagnosis, run `scripts\install-computer-use-local.ps1 -StrictVerifyOnly`. When the matching repair is authorized, run `-VerifyOnly` to repair, then `-StrictVerifyOnly` to verify. Do not patch `app.asar` or `resources\codex.exe` for this class unless Desktop logs also prove a UI availability gate is still closed.
